...
#     E.g.: docker.io/hexpm/elixir:1.19.5-erlang-28.3.3-debian-trixie-20260803-slim
...
# Campaign/match saves live here (config/runtime.exs SAVE_DIR default); the
# GameStore mkdir_p!'s it at boot, which must succeed as the `nobody` user.
# Mount a volume at this path to keep games across container replacements.
RUN mkdir -p /var/lib/hex_empire/saves \
  && chown -R nobody /var/lib/hex_empire

# set runner ENV
ENV MIX_ENV="prod"

# Only copy the final release from the build stage
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/hex_empire ./
